...
BEGIN
    -- Khai báo các biến
    DECLARE
@GiaTien FLOAT;                      -- Giá tiền của sản phẩm
    DECLARE
@ThanhTien DECIMAL(18,2);           -- Thành tiền
    DECLARE
@TrangThaiHDCT NVARCHAR(50);        -- Trạng thái của chi tiết hóa đơn
    DECLARE
...
BEGIN
        RAISERROR
('Số lượng tồn kho không đủ để thêm vào hóa đơn.', 16, 1);
        RETURN;
END

    -- Đặt trạng thái hóa đơn chi tiết là 'Chờ thanh toán'
    SET
@TrangThaiHDCT = N'Chờ thanh toán';
...
@ThanhTien = @SoLuong * @GiaTien;
    ...
    IF
EXISTS (SELECT 1 FROM HoaDonChiTiet WHERE MaHD = @MaHoaDon AND MaSP = @MaSanPham)
BEGIN
...
CREATE
OR
ALTER PROCEDURE THANHTOANHOADON
    @MAHOADON NVARCHAR(50),
    @TONGTIEN DECIMAL (18,2),
    @MATHANHTOAN INT,
    @MAKH NVARCHAR(50)
...
CREATE
OR
ALTER PROCEDURE CHINHSUASOLUONGSP
...
SET
@ThanhTien = @GiaTien * @SoLuongThayDoi;
...
CREATE
OR
ALTER PROCEDURE duaHDCTVeTrangThaiHuy
...
@TongTien DECIMAL(18, 2);
    SET
@TongTien = (SELECT SUM(GiaTien * SoLuongMua)
                     FROM HoaDonChiTiet 
